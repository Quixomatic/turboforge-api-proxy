...
export const validateResearchRequest = (req, res, next) => {
  const schema = Joi.object({
    processType: Joi.string().required().trim().min(3).max(100)
      .messages({
    ...
    industry: Joi.string().required().trim().min(3).max(100)
      .messages({
    ...
    additionalRequirements: Joi.string().allow('').optional().max(1000)
      .messages({
...
export const validateImplementRequest = (req, res, next) => {
  const schema = Joi.object({
    process: Joi.object({
      ...
      table: Joi.string().optional().default('incident')
        .messages({
    ...
    milestones: Joi.array().items(
      Joi.object({
        ...
        short_description: Joi.string().allow('').optional().max(200)
          .messages({
            'string.max': 'Milestone description must be at most 200 characters'
          }),
        glyph: Joi.string().allow('').optional().max(50)
          .messages({
            'string.max': 'Glyph must be at most 50 characters'
          }),
        order: Joi.number().integer().optional().default(100)
          .messages({
        ...
        steps: Joi.array().items(
          Joi.object({
            ...
            short_label: Joi.string().allow('').optional().max(50)
              .messages({
                'string.max': 'Short label must be at most 50 characters'
              }),
            step_type: Joi.string().optional().default('form')
              .valid('form', 'confirmation', 'yes/no', 'repeater', 'repeater_summary', 'repeater_summary_with_questions', 'dead_end')
              .messages({
                'any.only': 'Step type must be a valid type'
              }),
            display_label: Joi.string().allow('').optional().max(100)
              .messages({
            ...
            footer_message: Joi.string().allow('').optional().max(500)
              .messages({
            ...
            questions: Joi.array().items(
              Joi.object({
                ...
                label: Joi.string().required().trim().min(3).max(100)
                  .messages({
                ...
                type: Joi.string().optional().default('string')
                  .valid('string', 'integer', 'decimal', 'boolean', 'reference', 'choice', 'multiple_choice', 'date', 'datetime', 'container', 'multi_row_variable_set')
                  .messages({
                ...
                help_text: Joi.string().allow('').optional().max(500)
                  .messages({
                    'string.max': 'Help text must be at most 500 characters'
                  }),
                reference: Joi.string().allow('').optional().max(100)
                  .messages({
                    'string.max': 'Reference must be at most 100 characters'
                  }),
                reference_qual: Joi.string().allow('').optional().max(500)
                  .messages({
                    'string.max': 'Reference qualifier must be at most 500 characters'
                  }),
                sub_type: Joi.string().allow('').optional().max(100)
                  .messages({
                    'string.max': 'Sub type must be at most 100 characters'
                  }),
                value_field: Joi.string().allow('').optional().max(100)
                  .messages({
                    'string.max': 'Value field must be at most 100 characters'
                  }),
                default_value: Joi.string().allow('').optional().max(200)
                  .messages({
...
export const validateStatusRequest = (req, res, next) => {
  const schema = Joi.object({
    operationId: Joi.string().required().trim().guid()
      .messages({
...
export const validateCallbackRequest = (req, res, next) => {
  ...
  const bodySchema = Joi.object({
    success: Joi.boolean().required()
      .messages({
        'any.required': 'Success status is required'
      }),
